...
.DESCRIPTION
    ...
    temporary local migration account inside each guest through VMware Tools,
    ...
    VMware guest operations require guest OS administrator credentials. The
    vCenter credential is used for PowerCLI/vCenter access; the guest credential
    is used by Invoke-VMScript to read Windows NIC settings and create the local
    Hypervmigrate account.
...
function Invoke-VMwareMigrationDataCollection {
...
if (-not $GuestCredential) {
    $GuestCredential = Get-Credential -Message 'Enter guest OS administrator credentials for VMware Tools operations'
}
...
try {
    ...
    foreach ($name in $vmNames) {
        ...
        try {
            ...
            $guestNics = @()
            try {
                Write-Log -Message ("Collecting guest NIC details from '{0}' through VMware Tools." -f $vm.Name)
...
Export-ModuleMember -Function Invoke-VMwareMigrationDataCollection
